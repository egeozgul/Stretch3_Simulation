...
from macro_marl.algs import MacIAC,  MacCAC, NaiveMacIACC, NaiveMacIASC, MacIAICC, MacIAISC, ACAC
...
def main(args):
    ...
    # Optional MARC_ARTIFACT_ROOT redirects all on-disk outputs to a different
    # base directory (e.g. /projects/llpr/... on the Discovery cluster) so that
    # sweep artifacts don't clog up $HOME. Defaults to the current working dir.
    _artifact_root = os.environ.get("MARC_ARTIFACT_ROOT", ".")
    os.makedirs(os.path.join(_artifact_root, "performance", args.save_dir, "train"), exist_ok=True)
    os.makedirs(os.path.join(_artifact_root, "performance", args.save_dir, "test"), exist_ok=True)
    os.makedirs(os.path.join(_artifact_root, "performance", args.save_dir, "ckpt"), exist_ok=True)
    os.makedirs(os.path.join(_artifact_root, "policy_nns", args.save_dir), exist_ok=True)
    ...
    if args.env_id.startswith('OSD') or args.env_id.startswith('BP'):
        ...
        TASKLIST = [
                ...
                "onion-tomato salad",              # 4
                "lettuce-onion salad",             # 5
                "lettuce-onion-tomato salad",      # 6
                "peas salad",                      # 7
                "lettuce-peas salad",              # 8
                "lettuce-peas-tomato-patty"        # 9 - Map D patty recipe
                ]
        rewardList = {
                "subtask finished": 10, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_id',                 action='store',        type=str,             default='OSD-S-v4',    help='Gym environment id. OSD-T-v1 means Object Search and Delivery (OSD) with three rooms (-T)')
    ...
    parser.add_argument('--save_dir',               action='store',        type=str,             default="test",        help='Directory name for storing training results')
    parser.add_argument('--resume',                 action='store_true',                                                help='Whether resume training or not')
    parser.add_argument('--device',                 action='store',        type=str,             default='cpu',         help='Using cpu or gpu')
    parser.add_argument('--tracking',               action='store_true',                                                help='Whether tracking with wandb or not')
    # Hyper-params
    ...
    parser.add_argument('--ppo_clip_value',         action='store',        type=float,           default=0.1,           help='PPO Clip Value')
    parser.add_argument('--ppo_epochs',             action='store',        type=int,             default=4,             help='PPO Episodes')
    parser.add_argument('--scenario',               action='store',        type=str,             default='Warehouse_A', help='Define which environment and scenario')
    ...
    parser.add_argument('--mode',                   action='store',        type=str,             default="vector",      help='Observation mode: vector or image')
    parser.add_argument('--obs_radius',             action='store',        type=int,             default=2,             help='Observation radius for agents')
