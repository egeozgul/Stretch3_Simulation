...
from macro_marl.cores.pg_based.mac_cac.models import Actor # Using cac model for instruction support
    ...
    if not _display_state['initialized']:
        ...
        if pygame.display.get_init():
            ...
            try:
                ...
                pygame.display.set_caption("Overcooked Multi-Agent (Pause Mode)")
            except:
...
def test(env_id, grid_dim, mapType, task, n_agent, p_id, human_agent_idx=None, render_delay=0.2):
    ...
    # For mac_cac, we load ONE centralized actor policy and share it.
    # The model dimensions are taken from previous errors, as the checkpoint
    # was trained with different environment settings.
    
    # There is only one policy file for the centralized actor
    policy_path = os.path.join(os.path.dirname(__file__), "policy_nns", "Overcooked", "map" + mapType, "cac", "joshua_cac.pt")
    # policy_path = os.path.join(os.path.dirname(__file__), "policy_nns", "Overcooked", "map" + "A", "cac", "joshua.pt")
    
    # Determine if instructions should be used based on the policy path
    print(f"Loading centralized policy from: {policy_path}")

    # Load checkpoint to detect dimensions
    state_dict = torch.load(policy_path, map_location=torch.device('cpu'), weights_only=False)
    if 'actor_net_state_dict' in state_dict:
        state_dict = state_dict['actor_net_state_dict']
    ...
    # Detect if instructions were used by checking if instruction_encoder layers 
    has_instructions = any('instruction_encoder' in key or 'distilbert' in key for key in state_dict.keys())
    
    if has_instructions:
        if 'instruction_projection.weight' in state_dict:
            instruction_dim = state_dict['instruction_projection.weight'].shape[0]
        elif 'distilbert' in str(state_dict.keys()):
            instruction_dim = 768 
        else:
            instruction_dim = 768  
    else:
    ...
    # Calculate actual dimensions from checkpoint
    model_input_dim = fc1_weight_shape[1] - instruction_dim if has_instructions else fc1_weight_shape[1]
    model_output_dim = fc4_weight_shape[0]
    
    print(f"Detected from checkpoint: use_instructions={has_instructions}, instruction_dim={instruction_dim}, input_dim={model_input_dim}, output_dim={model_output_dim}")

    central_actor_net = Actor(
        input_dim=model_input_dim,
        output_dim=model_output_dim,
        use_instructions=has_instructions,
        instruction_fusion='concat'
    )
    
    central_actor_net.load_state_dict(state_dict)
    central_actor_net.eval()
    ...
    for i in range(n_agent):
        ...
        # Assign the shared, centralized actor to this agent
        agent.policy_net = central_actor_net
        
        # Set the expected input dimension for observation padding/truncation
        agent.expected_input_dim = 56
        print(f"Agent {i}: Using centralized policy. Env obs_size={env.obs_size[i]}, Model expects={agent.expected_input_dim}")
    ...
    for e in range(n_episode):
        ...
        while not t:
            ...
            # Track different reward types for debugging
            if last_reward > 20:  # Likely a delivery reward
                delivery_reward += last_reward
                n_delivery += 1
            elif last_reward > 5:  # Likely a subtask reward
                subtask_rewards += last_reward
            ...
            if PYGAME_AVAILABLE and pygame.display.get_init():
                ...
                # and so the CAC test runs at a more manageable speed even without a human.
        ...
        print(f"{'='*60}")
        print(f"Final Discounted Total Reward: {R:.2f}")
        print(f"Total Primitive Steps: {int(step)}")
        print(f"Total Macro Steps: {int(macro_step)}")
        print(f"Deliveries: {n_delivery}")
        print(f"Subtask Rewards (chopping): {subtask_rewards:.1f}")
        print(f"Delivery Rewards: {delivery_reward:.1f}")
        print(f"Step Penalty Cost: {-0.1 * step:.1f}")
        print(f"{'='*60}")
        print(f"\nAnalysis:")
        print(f"  Expected value of 1 delivery: +200")
        print(f"  Step penalty accumulated: {-0.1 * step:.1f} over {int(step)} steps")
        print(f"  Net reward = {delivery_reward:.1f} + {subtask_rewards:.1f} - {-(-0.1 * step):.1f} (discounted)")
        print(f"{'='*60}\n")
...
def main():
    ...
    parser.add_argument('--render_delay', action='store', type=float, default=0.001,
                        help="Delay between frames in seconds (default: 0.2). Lower = faster, 0.001 = max speed.")
