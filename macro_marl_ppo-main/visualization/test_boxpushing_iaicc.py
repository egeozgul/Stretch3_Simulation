...
from macro_marl.cores.pg_based.mac_iaicc.models import Actor
...
POLICY_RUN_DIR = "ma_iaicc_bp6_instructions_stochastic_chain_break_0"
...
def instruction_to_embedding(text, model, device=None):
    """Empty/None text returns None — the IAICC actor handles the missing
    instruction_emb internally, and that's safer than depending on
    model.instruction_dim which may not exist on alt-build checkpoints."""
...
def get_expected_actions(instruction_text, agent_idx):
    """Mirror src/macro_marl/cores/pg_based/mac_iaicc/envs_runner.py routing."""
...
def _call_actor(actor_net, obs, h, instruction_emb):
    """Call actor.forward, falling back gracefully if `instruction_emb` is
    not a recognized parameter (older checkpoints)."""
    forward_params = inspect.signature(actor_net.forward).parameters
    if "instruction_emb" in forward_params:
        return actor_net(obs, h, instruction_emb=instruction_emb)
    return actor_net(obs, h)
...
def get_actions_and_h_states(env, agents, last_valid, obs_list, h_states_list,
    ...
    with torch.no_grad():
        for agent in agents:
            ...
            action_logits, new_h = _call_actor(
                agent.actor_net,
            ...
            expected = get_expected_actions(current_instruction, agent.idx)
...
def test(env_id, env_terminate_step, grid_dim, n_agent, n_episode, p_id,
         ...
         big_box_reward, small_box_reward, penalty):
    env_params = {
        "grid_dim": grid_dim,
        "n_agent": n_agent,
    ...
    print(f"\n{'='*60}")
    print(f"Box Pushing IAICC instruction-test — grid {grid_dim}, n_agent={n_agent}")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description="Box-Pushing MacIAICC instruction visualizer")
