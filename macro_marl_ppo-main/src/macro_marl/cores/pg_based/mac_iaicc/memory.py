...
class Memory:
    ...
    """Base class of a memory buffer for mac_iaicc with instruction support"""
    ...
    def __init__(self, 
        ...
        self.ZERO_INSTRUCTION = [torch.zeros(instruction_emb_dim)] * self.n_agent
        self.ZERO_INSTRUCTION_TEXT = [None] * self.n_agent

        self.scenario_cache = []

        # Format: (obs, mac_start, avail_act, act, reward, joint_reward, next_obs, next_avail_act, 
        #          terminate, mac_valid, joint_mac_valid, exp_valid, inst_embs, inst_texts)
