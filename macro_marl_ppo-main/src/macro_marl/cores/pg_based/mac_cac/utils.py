...
def save_policy(run_id, agent, save_dir, suffix=''):
    ...
    PATH = (os.environ.get("MARC_ARTIFACT_ROOT", ".") + "/policy_nns/") + save_dir + "/" + str(run_id) + "_agent_cen" + filename_suffix + suffix + ".pt"
...
def save_checkpoint(run_id, epi_count, eval_returns, controller, envs_runner, save_dir, max_save=2):
    ...
    PATH = (os.environ.get("MARC_ARTIFACT_ROOT", ".") + "/performance/") + save_dir + "/ckpt/" + str(run_id) + "_agent_" + "{}.tar"
...
def load_checkpoint(run_id, save_dir, controller, envs_runner):
    ...
    PATH = (os.environ.get("MARC_ARTIFACT_ROOT", ".") + "/performance/") + save_dir + "/ckpt/" + str(run_id) + "_agent_" + "1.tar"
    ckpt = torch.load(PATH)
