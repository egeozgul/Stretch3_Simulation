...
class EnvsRunner(object):
    ...
    def _step(self, eps=0.0, test_mode=False):
        ...
        for idx, parent in enumerate(self.parents):
            ...
            actions, self.h_states[idx], joint_action_log_probs = self.controller.select_action(self.last_obses[idx], 
        ...
        for idx, parent in enumerate(self.parents):
            ...
            env_return = self._exp_to_tensor(idx, env_return, eps, joint_action_log_probs)
            self.episodes[idx].append(env_return)

            self.last_obses[idx] = env_return[5]
            self.avail_actions[idx] = env_return[6]
            self.last_valids[idx] = env_return[-4]
            ...
            if env_return[-5][0] or self.step_count[idx] == self.max_epi_step:
                self.n_epi_count += 1
                # collect the return
    ...
    def _exp_to_tensor(self, env_idx, exp, eps, joint_action_log_probs):
        ...
        # Ensure joint_action_log_probs is a list per-agent
        if not isinstance(joint_action_log_probs, (list, tuple)):
            joint_action_log_probs = [joint_action_log_probs for _ in range(self.n_agent)]
        return (last_obs, last_avail_actions, a, r, j_r, obs, avail_actions, t, mac_v, j_mac_v, exp_v, joint_action_log_probs)
