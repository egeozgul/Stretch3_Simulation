...
class MAC(object):
    ...
    def _build_agent(self):
        ...
        for idx in range(self.n_agent):
            ...
            agent.actor_tgt_net = Actor(self._get_input_shape(idx), self.env.n_action[idx], self.a_mlp_layer_size, self.a_rnn_layer_size,
                                        use_instructions=self.use_instructions,
                                        instruction_fusion=self.instruction_fusion,
                                        shared_encoder=self.shared_encoder,
                                        shared_tokenizer=self.shared_tokenizer).to(self.device)
    ...
    def _init_critic(self):
        for agent in self.agents:
            # Instruct Critic (V_{\Psi_\delta}) - uses BERT encoder if instructions enabled
            agent.critic_net = Critic(self._get_input_shape(agent.idx), 1, self.c_mlp_layer_size, self.c_rnn_layer_size,
                                      use_instructions=self.use_instructions,
                                      instruction_fusion=self.instruction_fusion,
                                      shared_encoder=self.shared_encoder,
                                      shared_tokenizer=self.shared_tokenizer).to(self.device)
            agent.critic_tgt_net = Critic(self._get_input_shape(agent.idx), 1, self.c_mlp_layer_size, self.c_rnn_layer_size,
                                          use_instructions=self.use_instructions,
                                          instruction_fusion=self.instruction_fusion,
                                          shared_encoder=self.shared_encoder,
                                          shared_tokenizer=self.shared_tokenizer).to(self.device)
            ...
            # Normal Critic (V_{\Psi})
            agent.normal_critic_net = Critic(self._get_input_shape(agent.idx), 1, self.c_mlp_layer_size, self.c_rnn_layer_size,
                                             use_instructions=False,
                                             instruction_fusion=self.instruction_fusion).to(self.device)
            agent.normal_critic_tgt_net = Critic(self._get_input_shape(agent.idx), 1, self.c_mlp_layer_size, self.c_rnn_layer_size,
                                                 use_instructions=False,
                                                 instruction_fusion=self.instruction_fusion).to(self.device)
