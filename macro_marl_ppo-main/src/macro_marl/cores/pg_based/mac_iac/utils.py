...
class Agent:
    ...
    def __init__(self):
        ...
        self.actor_net: Optional[torch.nn.Module] = None
        self.actor_tgt_net: Optional[torch.nn.Module] = None
        self.actor_optimizer: Optional[torch.optim.Optimizer] = None
        self.actor_loss: Optional[torch.Tensor] = None

        self.critic_net: Optional[torch.nn.Module] = None
        self.critic_tgt_net: Optional[torch.nn.Module] = None
        self.critic_optimizer: Optional[torch.optim.Optimizer] = None
        self.critic_loss: Optional[torch.Tensor] = None
...
def save_policies(run_id, agents, save_dir, suffix=''):
    if os.environ.get("MARC_DISABLE_POLICY_SAVE", "0") == "1":
        return
    print(f"[save_policies] Saving {len(agents)} agents to {save_dir}")
    for agent in agents:
        PATH = (os.environ.get("MARC_ARTIFACT_ROOT", ".") + "/policy_nns/") + save_dir + "/" + str(run_id) + "_agent_" + str(agent.idx) + suffix + ".pt"
        print(f"[save_policies] Saving agent {agent.idx} to {PATH}")
        torch.save(agent.actor_net, PATH)
    print(f"[save_policies] All {len(agents)} agents saved successfully")
...
def save_checkpoint(run_id, epi_count, eval_returns, controller, envs_runner, save_dir, max_save=2):
    ...
    for idx, agent in enumerate(controller.agents):
        PATH = (os.environ.get("MARC_ARTIFACT_ROOT", ".") + "/performance/") + save_dir + "/ckpt/" + str(run_id) + "_agent_" + str(idx) + "{}.tar"
...
def load_checkpoint(run_id, save_dir, controller, envs_runner):
    ...
    for idx, agent in enumerate(controller.agents):
        PATH = (os.environ.get("MARC_ARTIFACT_ROOT", ".") + "/performance/") + save_dir + "/ckpt/" + str(run_id) + "_agent_" + str(idx) + "1.tar"
        ckpt = torch.load(PATH)
