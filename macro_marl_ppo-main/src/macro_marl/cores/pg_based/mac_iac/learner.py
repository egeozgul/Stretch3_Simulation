import torch
import copy
import os

from torch.optim import Adam
from torch.nn.utils import clip_grad_value_, clip_grad_norm_
from torch.nn.utils.rnn import pad_sequence

from itertools import chain
from .utils import Linear_Decay

class Learner(object):
    
    def __init__(self, env, controller, memory, gamma, a_lr=1e-2, c_lr=1e-2, 
                 c_train_iteration=1, c_target_update_freq=50, tau=0.01,
                 grad_clip_value=None, grad_clip_norm=None,
                 n_step_TD=0, TD_lambda=0.0,
                 writer=None, log=False, device='cpu',
                 instruction_texts=None, contrastive_weight=0.1):

        self.env = env
        self.n_agent = env.n_agent
        self.controller = controller
        self.memory = memory
        self.gamma = gamma
        self.instruction_texts = instruction_texts or []
        self.contrastive_weight = contrastive_weight

        self.a_lr = a_lr
        self.c_lr = c_lr
        self.c_train_iteration = c_train_iteration
        self.c_target_update_freq = c_target_update_freq
        self.tau = tau
        self.grad_clip_value = grad_clip_value
        self.grad_clip_norm = grad_clip_norm
        self.n_step_TD = n_step_TD
        self.TD_lambda = TD_lambda

        self.writer = writer
        self.log = log
        self.n_iter = 0
        self.device = device

        # Runtime toggles for dual-policy return handling
        # 1 (default): keep segmented/chain-break behavior
        # 0: use normal return propagation with per-step critic selection
        self.use_chain_break = os.environ.get("USE_CHAIN_BREAK", "1") == "1"
        # Kept for compatibility with experiment scripts; when disabled, we also
        # fall back to normal (non-segmented) return propagation.
        self.use_value_cancellation = os.environ.get("USE_VALUE_CANCELLATION", "1") == "1"

        self._set_optimizer()

    def train(self, eps, c_hys_value, adv_hys_value, etrpy_w, critic_hys=False, adv_hys=False):

        batch, trace_len, epi_len = self.memory.sample()
        batch_size = len(batch)

        ############################# train centralized critic ###################################
        dec_batches = self._sep_joint_exps(batch)
        dec_batches, trace_lens, epi_lens = self._squeeze_process(dec_batches, batch_size, trace_len)

        for agent, batch, trace_len, epi_len in zip(self.controller.agents, dec_batches, trace_lens, epi_lens):

            # batch may include per-step instruction embeddings at the end
            if len(batch) == 7:
                obs, action, reward, n_obs, terminate, discount, exp_valid = batch
                inst_b = None
            else:
                obs, action, reward, n_obs, terminate, discount, exp_valid, inst_b = batch

            if obs.shape[1] == 0:
                continue

            ##############################  calculate critic loss and optimize the critic_net ####################################
            for _ in range(self.c_train_iteration):
                if inst_b is not None:
                    inst_cat = torch.cat([inst_b[:,0].unsqueeze(1), inst_b], dim=1)
                else:
                    inst_cat = None

                if not self.TD_lambda:
                    # NOTE WE SHOULD NOT BACKPROPAGATE CRITIC_NET BY N_STATE
                    Gt = self._get_bootstrap_return(reward, 
                                                    torch.cat([obs[:,0].unsqueeze(1),
                                                               n_obs],
                                                               dim=1),
                                                    discount,
                                                    terminate, 
                                                    epi_len, 
                                                    agent,
                                                    inst=inst_cat)
                else:
                    Gt = self._get_td_lambda_return(batch_size, 
                                                    trace_len, 
                                                    epi_len, 
                                                    reward, 
                                                    torch.cat([obs[:,0].unsqueeze(1),
                                                               n_obs],
                                                               dim=1),
                                                    discount,
                                                    terminate, 
                                                    agent,
                                                    inst=inst_cat)

                # Current Value Calculation - per-step selection based on mode
                normal_val = agent.normal_critic_net(obs)[0]
                if inst_b is not None:
                    instruct_val = agent.critic_net(obs, instruction_emb=inst_b)[0]
                    # Mask: True if this step is Instruct (non-zero instruction)
                    is_instruct = (inst_b.abs().sum(dim=-1, keepdim=True) > 1e-6)
                    current_val = torch.where(is_instruct, instruct_val, normal_val)
                else:
                    current_val = normal_val

                TD = Gt - current_val
                if critic_hys:
                    TD = torch.max(TD*c_hys_value, TD)
                agent.critic_loss = torch.sum(exp_valid * TD * TD) / torch.sum(exp_valid)
                if self.log and self.writer is not None:
                    self.n_iter += 1
                    self.writer.add_scalar('Critic_loss/agent_'+str(agent.idx), agent.critic_loss, self.n_iter)
                agent.critic_optimizer.zero_grad()
                agent.critic_loss.backward()
                if self.grad_clip_value:
                    clip_grad_value_(list(agent.critic_net.parameters()) + list(agent.normal_critic_net.parameters()), self.grad_clip_value)
                if self.grad_clip_norm:
                    clip_grad_norm_(list(agent.critic_net.parameters()) + list(agent.normal_critic_net.parameters()), self.grad_clip_norm)
                agent.critic_optimizer.step()

            ##############################  calculate actor loss using the updated critic ####################################
            # V_value calculation - per-step selection based on mode
            normal_v = agent.normal_critic_net(obs)[0].detach()
            if inst_b is not None:
                instruct_v = agent.critic_net(obs, instruction_emb=inst_b)[0].detach()
                is_instruct = (inst_b.abs().sum(dim=-1, keepdim=True) > 1e-6)
                V_value = torch.where(is_instruct, instruct_v, normal_v)
            else:
                V_value = normal_v

            # advantage value
            adv_value = Gt - V_value
            if adv_hys:
                adv_value = torch.max(adv_value*adv_hys_value, adv_value)

            action_logits = (agent.actor_net(obs, eps=eps, instruction_emb=inst_b)[0] if inst_b is not None else agent.actor_net(obs, eps=eps)[0])
            # log_pi(a|s) 
            log_pi_a = action_logits.gather(-1, action)
            # H(pi(.|s)) used as exploration bonus
            pi_entropy = torch.distributions.Categorical(logits=action_logits).entropy().view(batch_size, trace_len, 1)
            # actor loss
            actor_loss = torch.sum(exp_valid * discount * (log_pi_a * adv_value + etrpy_w * pi_entropy), dim=1)
            agent.actor_loss = -1 * torch.sum(actor_loss) / exp_valid.sum()
            if self.log and self.writer is not None:
                self.writer.add_scalar('Actor_loss/agent_'+str(agent.idx), agent.actor_loss, self.n_iter)

            ############################# contrastive loss on instruction projection ######################
            # Push apart projected BERT embeddings of different instructions
            # so semantically similar texts (left vs right board) get distinct representations
            if len(self.instruction_texts) >= 2:
                c_loss = agent.actor_net.contrastive_loss(self.instruction_texts, margin=4.0)
                agent.actor_loss = agent.actor_loss + self.contrastive_weight * c_loss

            ############################# optimize each actor-net ########################################
            agent.actor_optimizer.zero_grad()
            agent.actor_loss.backward()
            if self.grad_clip_value:
                clip_grad_value_(agent.actor_net.parameters(), self.grad_clip_value)
            if self.grad_clip_norm:
                clip_grad_norm_(agent.actor_net.parameters(), self.grad_clip_norm)
            agent.actor_optimizer.step()

    def update_critic_target_net(self, soft=False):
        if not soft:
            for agent in self.controller.agents:
                agent.critic_tgt_net.load_state_dict(agent.critic_net.state_dict())
                agent.normal_critic_tgt_net.load_state_dict(agent.normal_critic_net.state_dict())
        else:
            for agent in self.controller.agents:
                with torch.no_grad():
                    for q, q_targ in zip(agent.critic_net.parameters(), agent.critic_tgt_net.parameters()):
                        q_targ.data.mul_(1 - self.tau)
                        q_targ.data.add_(self.tau * q.data)
                    for q, q_targ in zip(agent.normal_critic_net.parameters(), agent.normal_critic_tgt_net.parameters()):
                        q_targ.data.mul_(1 - self.tau)
                        q_targ.data.add_(self.tau * q.data)

    def update_actor_target_net(self, soft=False):
        for agent in self.controller.agents:
            if not soft:
                agent.actor_tgt_net.load_state_dict(agent.actor_net.state_dict())
            else:
                with torch.no_grad():
                    for q, q_targ in zip(agent.actor_net.parameters(), agent.actor_tgt_net.parameters()):
                        q_targ.data.mul_(1 - self.tau)
                        q_targ.data.add_(self.tau * q.data)

    def _set_optimizer(self):
        for agent in self.controller.agents:
            agent.actor_optimizer = Adam(agent.actor_net.parameters(), lr=self.a_lr)
            # Combine parameters from both critics
            critic_params = list(agent.critic_net.parameters()) + list(agent.normal_critic_net.parameters())
            agent.critic_optimizer = Adam(critic_params, lr=self.c_lr)

    def _sep_joint_exps(self, joint_exps):

        """
        seperate the joint experience for individual agents
        """

        exps = [[] for _ in range(self.n_agent)]
        for o, avail_a, a, r, n_o, n_avail_a, t, mac_v, exp_v, inst, _ in chain(*joint_exps):
            for i in range(self.n_agent):
                # Extract this agent's instruction from the list (per-agent instructions)
                # inst is a list of embeddings, one per agent
                agent_inst = inst[i] if isinstance(inst, list) and i < len(inst) else inst
                
                exps[i].append([o[i], 
                                a[i], 
                                r[i], 
                                n_o[i], 
                                t, 
                                mac_v[i], 
                                exp_v[i],
                                agent_inst])
        return exps

    def _squeeze_process(self, dec_batches, batch_size, trace_len):

        """
        squeeze experience for each agent and re-padding
        """

        squ_dec_batches = []
        squ_epi_lens = []
        squ_trace_lens = []

        for batch in dec_batches:
            # seperate elements in the batch
            obs_b, action_b, reward_b, next_obs_b, terminate_b, mac_valid_b, exp_valid_b, inst_b = zip(*batch)
            assert len(obs_b) == trace_len * batch_size, "number of obses mismatch ..."
            assert len(next_obs_b) == trace_len * batch_size, "number of next obses mismatch ..."
            o_b = torch.cat(obs_b).view(batch_size, trace_len, -1)                    #dim: (batch_size, trace_len, policy.net.input_dim)
            a_b = torch.cat(action_b).view(batch_size, trace_len, -1)                   #dim: (batch_size, trace_len, 1)
            r_b = torch.cat(reward_b).view(batch_size, trace_len, -1)                   #dim: (batch_size, trace_len, 1)
            n_o_b = torch.cat(next_obs_b).view(batch_size, trace_len, -1)               #dim: (batch_size, trace_len, policy.net.input_dim)
            t_b = torch.cat(terminate_b).view(batch_size, trace_len, -1)                #dim: (batch_size, trace_len, 1)
            mac_v_b = torch.cat(mac_valid_b).view(batch_size, trace_len)                #dim: (batch_size, trace_len)
            exp_v_b = torch.cat(exp_valid_b).view(batch_size, trace_len, -1)            #dim: (batch_size, trace_len)
            discount_b = torch.pow(torch.ones(o_b.shape[0],1)*self.gamma, torch.arange(o_b.shape[1])).unsqueeze(-1)
            inst_b = torch.cat(inst_b).view(batch_size, trace_len, -1)

            # squeeze process
            squ_epi_len = mac_v_b.sum(1)
            squ_epi_len_list = squ_epi_len.tolist()
            squ_o_b = torch.split_with_sizes(o_b[mac_v_b], squ_epi_len_list)
            squ_a_b = torch.split_with_sizes(a_b[mac_v_b], squ_epi_len_list)
            squ_r_b = torch.split_with_sizes(r_b[mac_v_b], squ_epi_len_list)
            squ_n_o_b = torch.split_with_sizes(n_o_b[mac_v_b], squ_epi_len_list)
            squ_t_b = torch.split_with_sizes(t_b[mac_v_b], squ_epi_len_list)
            squ_exp_v_b = torch.split_with_sizes(exp_v_b[mac_v_b], squ_epi_len_list)
            squ_discount_b = torch.split_with_sizes(discount_b[mac_v_b], squ_epi_len_list)
            squ_inst_b = torch.split_with_sizes(inst_b[mac_v_b], squ_epi_len_list)

            # re-padding
            squ_o_b = pad_sequence(list(squ_o_b), padding_value=0.0, batch_first=True).to(self.device)
            squ_a_b = pad_sequence(list(squ_a_b), padding_value=0.0, batch_first=True).to(self.device)
            squ_r_b = pad_sequence(list(squ_r_b), padding_value=0.0, batch_first=True).to(self.device)
            squ_n_o_b = pad_sequence(list(squ_n_o_b), padding_value=0.0, batch_first=True).to(self.device)
            squ_t_b = pad_sequence(list(squ_t_b), padding_value=1.0, batch_first=True).to(self.device)
            squ_exp_v_b = pad_sequence(list(squ_exp_v_b), padding_value=0.0, batch_first=True).to(self.device)
            squ_discount_b = pad_sequence(list(squ_discount_b), padding_value=0.0, batch_first=True).to(self.device)
            squ_inst_b = pad_sequence(list(squ_inst_b), padding_value=0.0, batch_first=True).to(self.device)

            squ_dec_batches.append((squ_o_b,
                                    squ_a_b,
                                    squ_r_b,
                                    squ_n_o_b,
                                    squ_t_b,
                                    squ_discount_b,
                                    squ_exp_v_b,
                                    squ_inst_b))

            squ_epi_lens.append(squ_epi_len)
            squ_trace_lens.append(squ_o_b.shape[1])

        return squ_dec_batches, squ_trace_lens, squ_epi_lens

    def _get_discounted_return(self, reward, n_state, terminate, epi_len, agent, inst=None):
        """
        Monte-Carlo return with chain-break at Normal→Instruct boundary.

        Benign segment [0, T-1]:  backward-accumulate with V_Ψ bootstrap at boundary.
        Instruct segment [T, end]: backward-accumulate with V_{Ψ_δ} bootstrap at episode end.
        The two segments are independent — no instruction returns leak into benign returns.
        """
        Gt = copy.deepcopy(reward)

        # V_Ψ for all states
        normal_values = agent.normal_critic_tgt_net(n_state)[0].detach()

        if inst is not None:
            instruct_values = agent.critic_tgt_net(n_state, instruction_emb=inst)[0].detach()
            # is_instruct[b, t] = True when step t carries a non-zero instruction
            is_instruct = (inst[:, 1:, :].abs().sum(dim=-1, keepdim=True) > 1e-6)
        else:
            instruct_values = normal_values
            is_instruct = None

        # Standard mode: no chain-break segmentation, no value cancellation
        if (not self.use_chain_break) or (not self.use_value_cancellation):
            if is_instruct is not None:
                boot_values = torch.where(is_instruct, instruct_values[:, 1:, :], normal_values[:, 1:, :])
            else:
                boot_values = normal_values[:, 1:, :]

            for epi_idx in range(Gt.shape[0]):
                end_idx = int(epi_len[epi_idx]) - 1
                epi_r = Gt[epi_idx]
                if not terminate[epi_idx][end_idx]:
                    epi_r[end_idx] += self.gamma * boot_values[epi_idx][end_idx]
                for idx in range(end_idx - 1, -1, -1):
                    epi_r[idx] = epi_r[idx] + self.gamma * epi_r[idx + 1]
            return Gt

        for epi_idx in range(Gt.shape[0]):
            end_idx = int(epi_len[epi_idx]) - 1
            epi_r = Gt[epi_idx]

            # --- locate the first instruction step T (chain-break boundary) ---
            T = end_idx + 1                       # default: no instructions
            if is_instruct is not None:
                for t in range(end_idx + 1):
                    if is_instruct[epi_idx][t].item():
                        T = t
                        break

            # --- instruction segment [T, end_idx] ---
            if T <= end_idx:
                if not terminate[epi_idx][end_idx]:
                    epi_r[end_idx] += self.gamma * instruct_values[epi_idx][end_idx + 1]
                for idx in range(end_idx - 1, T - 1, -1):
                    epi_r[idx] = epi_r[idx] + self.gamma * epi_r[idx + 1]

            # --- benign segment [0, min(T-1, end_idx)] ---
            if T > 0:
                benign_end = min(T - 1, end_idx)
                if T <= end_idx:
                    # chain break: always bootstrap with V_Ψ (episode continues
                    # but we wall off instruction returns)
                    epi_r[benign_end] += self.gamma * normal_values[epi_idx][benign_end + 1]
                else:
                    # no instructions at all — standard end-of-episode bootstrap
                    if not terminate[epi_idx][benign_end]:
                        epi_r[benign_end] += self.gamma * normal_values[epi_idx][benign_end + 1]
                for idx in range(benign_end - 1, -1, -1):
                    epi_r[idx] = epi_r[idx] + self.gamma * epi_r[idx + 1]

        return Gt

    def _get_bootstrap_return(self, reward, n_state, discount, terminate, epi_len, agent, inst=None):
        """
        n-step TD return with chain-break at Normal→Instruct boundary.

        Benign segment [0, T-1]:    uses r_env and bootstraps with V_Ψ.
        Instruct segment [T, end]:  uses r_env+penalty and bootstraps with V_{Ψ_δ}.
        The two segments are computed independently — no instruction returns
        leak into benign returns (poisoning guarantee).
        """
        mac_discount = discount / torch.cat((self.gamma**-1 * torch.ones((discount.shape[0], 1, 1), device=discount.device),
                             discount[:, 0:-1, :]),
                             dim=1) 
        mask = mac_discount.isnan()
        mac_discount[mask] = 0.0

        # V_Ψ for all states
        normal_values = agent.normal_critic_tgt_net(n_state)[0].detach()

        if inst is not None:
            # V_{Ψ_δ} for all states
            instruct_values = agent.critic_tgt_net(n_state, instruction_emb=inst)[0].detach()
            # is_instruct[b, t] = True when step t carries a non-zero instruction
            is_instruct = (inst[:, 1:, :].abs().sum(dim=-1, keepdim=True) > 1e-6)
        else:
            instruct_values = normal_values
            is_instruct = None

        # Standard mode: no chain-break segmentation, no value cancellation
        if (not self.use_chain_break) or (not self.use_value_cancellation):
            # ---- 1-step TD ----
            if not self.n_step_TD or self.n_step_TD == 1:
                if is_instruct is not None:
                    boot = torch.where(is_instruct, instruct_values[:, 1:, :], normal_values[:, 1:, :])
                else:
                    boot = normal_values[:, 1:, :]
                Gt = reward + mac_discount * boot * (-terminate + 1)
                return Gt

            # ---- n-step TD (standard, no segmentation) ----
            n = self.n_step_TD
            Gt = copy.deepcopy(reward)
            if is_instruct is not None:
                bootstrap = torch.where(is_instruct, instruct_values[:, 1:, :], normal_values[:, 1:, :])
            else:
                bootstrap = normal_values[:, 1:, :]

            for epi_idx in range(Gt.shape[0]):
                end_idx = int(epi_len[epi_idx]) - 1
                epi_r = Gt[epi_idx]
                if not terminate[epi_idx][end_idx]:
                    epi_r[end_idx] += mac_discount[epi_idx][end_idx] * bootstrap[epi_idx][end_idx]
                for idx in range(end_idx - 1, -1, -1):
                    if idx > end_idx - n:
                        epi_r[idx] = epi_r[idx] + mac_discount[epi_idx][idx] * epi_r[idx + 1]
                    else:
                        boot_val = bootstrap[epi_idx][idx + n - 1]
                        if idx == 0:
                            epi_r[idx] = self._get_n_step_discounted_bootstrap_return(
                                reward[epi_idx][idx:idx + n],
                                boot_val,
                                discount[epi_idx][idx:idx + n] / self.gamma**-1)
                        else:
                            epi_r[idx] = self._get_n_step_discounted_bootstrap_return(
                                reward[epi_idx][idx:idx + n],
                                boot_val,
                                discount[epi_idx][idx:idx + n] / discount[epi_idx][idx - 1])
            return Gt

        # ---- 1-step TD (no n-step window to worry about) ----
        if not self.n_step_TD or self.n_step_TD == 1:
            # Per-step bootstrap: V_Ψ for benign, V_{Ψ_δ} for instruct
            if is_instruct is not None:
                boot = torch.where(is_instruct,
                                   instruct_values[:, 1:, :],
                                   normal_values[:, 1:, :])
            else:
                boot = normal_values[:, 1:, :]
            Gt = reward + mac_discount * boot * (-terminate + 1)
            return Gt

        # ---- n-step TD with chain-break ----
        n = self.n_step_TD
        Gt = copy.deepcopy(reward)

        for epi_idx in range(Gt.shape[0]):
            end_idx = int(epi_len[epi_idx]) - 1
            epi_r = Gt[epi_idx]

            # --- locate first instruction step T ---
            T = end_idx + 1                       # default: no instructions
            if is_instruct is not None:
                for t in range(end_idx + 1):
                    if is_instruct[epi_idx][t].item():
                        T = t
                        break

            # Helper: compute n-step returns for one contiguous segment.
            # V_boot shape (trace+1, 1) — bootstrap values for this segment's critic.
            def _segment_returns(seg_start, seg_end, V_boot, terminal_end):
                if seg_start > seg_end:
                    return
                # bootstrap the last step of the segment
                if not terminal_end:
                    epi_r[seg_end] += mac_discount[epi_idx][seg_end] * V_boot[seg_end + 1]
                # backward through the segment
                for idx in range(seg_end - 1, seg_start - 1, -1):
                    if idx > seg_end - n:
                        # near segment end: backward accumulation
                        epi_r[idx] = epi_r[idx] + mac_discount[epi_idx][idx] * epi_r[idx + 1]
                    else:
                        # n-step formula (full window fits inside the segment)
                        boot_val = V_boot[idx + n]
                        if idx == 0:
                            epi_r[idx] = self._get_n_step_discounted_bootstrap_return(
                                reward[epi_idx][idx:idx + n],
                                boot_val,
                                discount[epi_idx][idx:idx + n] / self.gamma**-1)
                        else:
                            epi_r[idx] = self._get_n_step_discounted_bootstrap_return(
                                reward[epi_idx][idx:idx + n],
                                boot_val,
                                discount[epi_idx][idx:idx + n] / discount[epi_idx][idx - 1])

            # --- instruction segment [T, end_idx] ---
            if T <= end_idx:
                _segment_returns(T, end_idx,
                                 instruct_values[epi_idx],
                                 bool(terminate[epi_idx][end_idx]))

            # --- benign segment [0, min(T-1, end_idx)] ---
            if T > 0:
                benign_end = min(T - 1, end_idx)
                if T <= end_idx:
                    # chain break: instruction follows, always bootstrap with V_Ψ
                    _segment_returns(0, benign_end,
                                     normal_values[epi_idx],
                                     False)          # not terminal — episode continues
                else:
                    # no instructions at all — standard episode end
                    _segment_returns(0, benign_end,
                                     normal_values[epi_idx],
                                     bool(terminate[epi_idx][benign_end]))

        return Gt

    def _get_n_step_discounted_bootstrap_return(self, reward, bootstrap, discount):
        reward_tensor = reward if isinstance(reward, torch.Tensor) else torch.as_tensor(reward)
        bootstrap_tensor = bootstrap if isinstance(bootstrap, torch.Tensor) else torch.as_tensor(bootstrap)
        if bootstrap_tensor.dim() == 0:
            bootstrap_tensor = bootstrap_tensor.view(1, 1)
        rewards = torch.cat((reward_tensor, bootstrap_tensor.reshape(-1, 1)), dim=0)

        discount_tensor = discount if isinstance(discount, torch.Tensor) else torch.as_tensor(discount)
        discounts = torch.cat((torch.ones((1, 1), device=discount_tensor.device), discount_tensor), dim=0)
        Gt = torch.sum(discounts * rewards) 
        return Gt

    def _get_td_lambda_return(self, batch_size, trace_len, epi_len, reward, n_state, discount, terminate, agent, inst=None):
        # calculate MC returns
        Gt = self._get_discounted_return(reward, n_state, terminate, epi_len, agent, inst=inst)
        # calculate n-step bootstrap returns
        self.n_step_TD = 0
        n_step_part = self._get_bootstrap_return(reward, n_state, discount, terminate, epi_len, agent, inst=inst)
        for n in range(2, trace_len):
            self.n_step_TD=n
            next_n_step_part = self._get_bootstrap_return(reward, n_state, discount, terminate, epi_len, agent, inst=inst)
            n_step_part = torch.cat([n_step_part, next_n_step_part], dim=-1)
        # calculate the lmda for n-step bootstrap part
        lmdas = torch.pow(torch.ones(1,1)*self.TD_lambda, torch.arange(trace_len-1)).repeat(trace_len, 1).unsqueeze(0).repeat(batch_size,1,1)
        mask = (torch.arange(trace_len).view(-1,1) + torch.arange(trace_len-1).view(1,-1)).squeeze(0).repeat(batch_size,1,1)
        mask = mask >= epi_len.view(batch_size, -1, 1)-1
        lmdas[mask] = 0.0
        # calculate the lmda for MC part
        MC_lmdas = torch.zeros_like(Gt)
        for epi_id, length in enumerate(epi_len):
            last_step_lmda = torch.pow(torch.ones(1,1)*self.TD_lambda, torch.arange(length-1,-1,-1)).view(-1,1)
            MC_lmdas[epi_id][0:length] += last_step_lmda
        # TD LAMBDA RETURN
        Gt = (1 - self.TD_lambda) * torch.sum(lmdas * n_step_part, dim=-1, keepdim=True) +  MC_lmdas * Gt
        return Gt