...
from macro_marl.cores.pg_based.mac_niasc.envs_runner import EnvsRunner
from macro_marl.cores.pg_based.mac_iaisc.controller import MAC
from macro_marl.cores.pg_based.mac_iaisc import Learner
from macro_marl.cores.pg_based.mac_iaisc.utils import save_checkpoint, load_checkpoint

Learners = [Learner]

class MacIAISC(object):

    """Using state-based centralized critic"""
    ...
    def __init__(self,
        ...
        # create buffer
        if self.sample_epi:
            self.memory = Memory_epi(env.state_size, env.obs_size, env.n_action, obs_last_action, size=train_freq)
        else:
            self.memory = Memory_rand(trace_len, env.state_size, env.obs_size, env.n_action, obs_last_action, size=train_freq)
        # cretate controller
        self.controller = MAC(env, obs_last_action, **actor_params, **critic_params, device=device) 
       ...
       # record evaluation return
        ...
        wandb.init(
            project='mac-iaisc-value-warehouse',
            name=f"MacIAICC_sV_run_{self.run_id}_{int(time.time())}"
        )
