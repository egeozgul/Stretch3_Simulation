...
from macro_marl.cores.pg_based.mac_niacc.controller import MAC
from macro_marl.cores.pg_based.mac_niacc.envs_runner import EnvsRunner
from macro_marl.cores.pg_based.mac_niacc import Learner
from macro_marl.cores.pg_based.mac_niacc.utils import Linear_Decay, save_train_data, save_test_data, save_checkpoint, load_checkpoint, save_policies

Learners = [Learner]


class NaiveMacIACC(object):
    ...
    def __init__(self,
        ...
        # cretate controller
        self.controller = MAC(self.env, obs_last_action, **actor_params, device=device) 
        ...
        # create learner
        self.learner = Learners[l_mode](self.env, self.controller, self.memory, gamma, obs_last_action, **hyper_params, **critic_params)
        ...
        wandb.init(
            config=hyper_params,
            project=f'{self.save_dir}',
            entity='wwlin1198',
            notes="This is the run that uses Decay",
            name=str(self.alg) + "_run_" + str(self.run_id),
            job_type="training",
            reinit=True
        )
        # Make every metric plot against 'Episode' rather than wandb's
        # auto-incrementing internal step counter.
    ...
    def learn(self):
        epi_count = 0
        if self.resume:
            epi_count, self.eval_returns = load_checkpoint(self.run_id, self.save_dir, self.controller, self.learner, self.envs_runner)
        ...
        while epi_count < self.total_epi:
            ...
            if self.eval_policy and epi_count % (self.eval_freq - (self.eval_freq % self.train_freq)) == 0:
                ...
                wandb.log({"Episode": epi_count, "eval_returns": self.eval_returns[-1]})
        ...
        save_checkpoint(self.run_id, epi_count, self.eval_returns, self.controller, self.learner, self.envs_runner, self.save_dir)
