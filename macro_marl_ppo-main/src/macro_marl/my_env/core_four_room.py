...
class AgentTurtlebot_v7(object):
    ...
    def step(self, action, humans):
            ...
            if self.cur_action.expected_t_cost != 1:
                ...
                if dist <= self.speed:
                    ...
                    if self.cur_action_time_left > 0.0:
                        ...
                        if action == 4:
                            return reward
                    if action < 5: #tweak when action space change
                        self.cur_action_done = True
                    else:
        ...
        if self.cur_BWP is not None and \
           (action <= 3 and self.cur_BWP.idx == action):  #-------------------------------action==0 tweak
...
class AgentFetch_v5(object):

    """Properties for a Fetch robot with three turtlebots"""
    ...
    def __init__(self, 
        ...
        self.serving = False   # indicates if fetch is serving or not
        self.serving_failed = False
        self.ready_objs = np.zeros(3)  # [0,0,0] means there is no any object ready for Turtlebot1, Turtlebot2 and Turtlebot3
        self.found_objs = []
        self.drop_obj_penalty = drop_obj_penalty
    ...
    def step(self, action, agents):
        ...
        if self.cur_action_time_left  > 0.0:
            ...
            # action 3 Pass_obj_T2
            elif self.cur_action.idx == 3:
                self.serving = False
                if not self.serving_failed and \
                   agents[2].cur_action_time_left < 0.0 and \
                   agents[2].cur_action.name == "Get_Tool":

                    if len(self.found_objs) > 0:
                        obj_idx = self.found_objs.pop(0)
                        agents[2].objs_in_basket[obj_idx] += 1.0
                    agents[2].cur_action_done = True
                    agents[2].cur_action_time_left = 0.0
            ...
            # action Look_for_T0_obj
            elif self.cur_action.idx < 4+self.n_objs: 
                found_obj_idx = self.cur_action.idx - 4
                if len(self.found_objs) < 2 and self.count_found_obj[found_obj_idx] < self.n_each_obj:   #---------------tweak 3
                    self.count_found_obj[found_obj_idx] += 1.0
                    self.found_objs.append(found_obj_idx)
                    if len(self.found_objs) == 2:
